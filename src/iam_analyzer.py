...
Auditoría de identidades y accesos en entornos cloud/on-premise simulados.
"""
...
class AccessKey:
    ...
    servicio_ultimo_uso: Optional[str] = None  # ej: "s3", "ec2"
...
class Usuario:
    ...
    tiene_consola: bool = True       # acceso a la consola web habilitado
    es_cuenta_servicio: bool = False  # cuenta de servicio / bot / CI
...
class ControlNoAuditado:
    """Registra controles que no pudieron ejecutarse por falta de permisos u otro error."""
    usuario_id: str
    control: str
    motivo: str
...
class AnalizadorIAM:
    ...
    def _verificar_mfa(self, usuario: Usuario):
        if not usuario.mfa_activo:
            tiene_rol_privilegiado = any(
                r.lower() in self.ROLES_PRIVILEGIADOS for r in usuario.roles
            )
            severidad = Severidad.CRITICA if tiene_rol_privilegiado else Severidad.ALTA
    ...
    def _verificar_inactividad(self, usuario: Usuario):
        ...
        dias_inactivo = (datetime.now() - usuario.ultimo_acceso).days
        if dias_inactivo > self.DIAS_INACTIVIDAD_UMBRAL:
            self.hallazgos.append(Hallazgo(
                severidad=Severidad.MEDIA,
                categoria="INACTIVIDAD",
                usuario_id=usuario.id,
                descripcion=f"{usuario.email} inactivo por {dias_inactivo} días",
    ...
    def _verificar_privilegios_excesivos(self, usuario: Usuario):
        roles_peligrosos = [r for r in usuario.roles if r.lower() in self.ROLES_PRIVILEGIADOS]
        if len(roles_peligrosos) > 0:
            self.hallazgos.append(Hallazgo(
                severidad=Severidad.ALTA,
                categoria="PRIVILEGIO_EXCESIVO",
                usuario_id=usuario.id,
                descripcion=f"{usuario.email} posee rol(es) de alto privilegio: {roles_peligrosos}",
    ...
    def _verificar_permisos_directos(self, usuario: Usuario):
        # Los permisos directos dificultan la auditoría y gobernanza
        if len(usuario.permisos_directos) > 0:
            self.hallazgos.append(Hallazgo(
                severidad=Severidad.MEDIA,
    ...
    def _verificar_access_keys(self, usuario: Usuario):
        ...
        for key in usuario.access_keys:
            ...
            dias_desde_creacion = (hoy - key.creada).days

            # Key activa sin rotar por más del umbral
            if dias_desde_creacion > self.DIAS_ROTACION_KEY:
                self.hallazgos.append(Hallazgo(
                    severidad=Severidad.ALTA,
                    categoria="ACCESS_KEY_ANTIGUA",
                    usuario_id=usuario.id,
                    descripcion=(
                        f"{usuario.email}: key {key.key_id[-4:]}... "
                        f"sin rotar hace {dias_desde_creacion} días"
                    ),
                    recomendacion=(
                        "Rotar la access key. Implementar rotación automática "
                        f"cada {self.DIAS_ROTACION_KEY} días."
                    ),
                    mitre_id="T1552.001"
                ))

            # Key activa pero sin usarse durante mucho tiempo
            if key.ultimo_uso is not None:
                dias_sin_uso = (hoy - key.ultimo_uso).days
                if dias_sin_uso > self.DIAS_KEY_SIN_USO:
                    self.hallazgos.append(Hallazgo(
                        severidad=Severidad.MEDIA,
                        ...
                        descripcion=(
                            f"{usuario.email}: key {key.key_id[-4:]}... "
                            f"activa pero sin uso hace {dias_sin_uso} días"
                        ),
                        recomendacion="Revocar la key si no está en uso activo. Credenciales inactivas amplían la superficie de ataque.",
                        mitre_id="T1078"
                    ))
            elif dias_desde_creacion > self.DIAS_KEY_SIN_USO:
                # Nunca fue usada y ya lleva tiempo creada
                self.hallazgos.append(Hallazgo(
                    severidad=Severidad.MEDIA,
                    categoria="ACCESS_KEY_INACTIVA",
                    usuario_id=usuario.id,
                    descripcion=(
                        f"{usuario.email}: key {key.key_id[-4:]}... "
                        f"nunca usada, creada hace {dias_desde_creacion} días"
                    ),
    ...
    def _verificar_rotacion_password(self, usuario: Usuario):
        ...
        if dias > self.DIAS_ROTACION_PASSWORD:
            self.hallazgos.append(Hallazgo(
                ...
                recomendacion=(
                    f"Forzar cambio de contraseña. Configurar password policy con rotación "
                    f"máxima de {self.DIAS_ROTACION_PASSWORD} días."
                ),
                mitre_id="T1110"
            ))

    def _verificar_cuenta_servicio_interactiva(self, usuario: Usuario):
        """Detecta cuentas de servicio con acceso a consola habilitado."""
        nombre_lower = usuario.nombre.lower()
        parece_servicio = any(nombre_lower.startswith(p) for p in self.PREFIJOS_CUENTA_SERVICIO)
        ...
        if (usuario.es_cuenta_servicio or parece_servicio) and usuario.tiene_consola:
            self.hallazgos.append(Hallazgo(
                ...
                recomendacion=(
                    "Deshabilitar el acceso a consola. Las cuentas de servicio "
                    "solo deben operar via API/keys, nunca via login interactivo."
                ),
                mitre_id="T1078.004"
            ))
